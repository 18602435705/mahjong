...
} from "./mock/initialDealPresets.js";
import type { InitialDealPresetId } from "./mock/initialDealPresets.js";
...
export type GameAction =
  ...
  | { type: typeof GAME_ACTION.PLAYER_DISCARD; actor: number; tile: Tile }
  | { type: typeof GAME_ACTION.PLAYER_SELF_HU; actor: number }
  | {
  ...
  | {
      type: typeof GAME_ACTION.PLAYER_CLAIM_DECISION;
      actor: number;
      accept: boolean;
 ...
 */
export const getPlayerTurnOptions = (state: GameState, actor: number) => {
  const player = state.players[actor];
  const canAct = state.phase === PHASE.PLAYER_TURN && state.currentPlayer === actor;
  const canUseDrawActions =
    canAct && player.justDrawnTile !== null && player.hand.length % 3 === 2;
  const selfHu = canUseDrawActions ? evaluateHu(player.hand, player.melds) : null;
  const selfHuMethod = selfHu ? getSelfHuMethod(state, actor) : null;
  const selfHuSpecials = selfHu ? getSelfHuSpecials(state, actor) : [];
  ...
  return {
    ...
    selfHuMethod,
    selfHuSpecials,
    anGangTiles: canUseDrawActions ? getAnGangOptions(player) : [],
    buGangTiles: canUseDrawActions ? getBuGangOptions(player) : [],
  };
};

/**
 * 汇总 0 号位玩家（传统本地人类位）的可执行回合动作。
 */
export const getHumanTurnOptions = (state: GameState) =>
  getPlayerTurnOptions(state, 0);
...
type PlayerDiscardAction = Extract<
  GameAction,
  { type: typeof GAME_ACTION.PLAYER_DISCARD }
>;
type PlayerSelfHuAction = Extract<
  GameAction,
  { type: typeof GAME_ACTION.PLAYER_SELF_HU }
>;
type PlayerGangAction = Extract<GameAction, { type: typeof GAME_ACTION.PLAYER_GANG }>;
type PlayerClaimDecisionAction = Extract<
  GameAction,
  { type: typeof GAME_ACTION.PLAYER_CLAIM_DECISION }
>;
type PlayerQiangGangDecisionAction = Extract<
  GameAction,
  { type: typeof GAME_ACTION.PLAYER_QIANG_GANG_DECISION }
>;
...
function canPlayerActInTurn(state: GameState, actor: number) {
  return state.phase === PHASE.PLAYER_TURN && state.currentPlayer === actor;
}
...
function reducePlayerDiscard(
  state: GameState,
  action: PlayerDiscardAction,
): GameState {
  if (!canPlayerActInTurn(state, action.actor)) {
    return state;
  }

  return discardTile(state, action.actor, action.tile);
}

function reducePlayerSelfHu(
  state: GameState,
  action: PlayerSelfHuAction,
): GameState {
  if (!canPlayerActInTurn(state, action.actor)) {
    return state;
  }

  return trySelfHu(state, action.actor);
}

function reducePlayerGang(state: GameState, action: PlayerGangAction): GameState {
  if (!canPlayerActInTurn(state, action.actor)) {
    return state;
  }

  if (action.gangType === MELD_TYPE.AN_GANG) {
    return tryAnGang(state, action.actor, action.tile);
  }

  return tryBuGang(state, action.actor, action.tile);
}

function reducePlayerClaimDecision(
  state: GameState,
  action: PlayerClaimDecisionAction,
): GameState {
  const claimOptions = getLeadingClaimsForPlayer(state, action.actor);
  ...
  return passLeadingClaimsForPlayer(state, action.actor);
}

function reducePlayerQiangGangDecision(
  state: GameState,
  action: PlayerQiangGangDecisionAction,
): GameState {
  const candidate = getCurrentQiangGangCandidate(state);
  if (candidate !== action.actor) {
    return state;
  }

  if (action.accept) {
    return acceptQiangGangHu(state, action.actor);
  }
...
export function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case GAME_ACTION.PLAYER_DISCARD:
      return reducePlayerDiscard(state, action);
    case GAME_ACTION.PLAYER_SELF_HU:
      return reducePlayerSelfHu(state, action);
    case GAME_ACTION.PLAYER_GANG:
      return reducePlayerGang(state, action);
